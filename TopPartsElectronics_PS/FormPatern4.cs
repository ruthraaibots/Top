...
{
    public partial class FormPatern4 : Form
    {
        ...
        private void FormPatern4_Load(object sender, EventArgs e)
        {
            this.Text = ProcessName;
            txtLotNo.Text = Get_Lotno_p4;
            txtPartNo.Text = Get_Partno_p4;
            txt_patern4_qty.Text = Get_Qty_p4;
        }
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if(checkInput())
                {
                    ...
                    {
                        string ActionType = "SaveDatafour";
                        string ActionType_lot = string.Empty;
                        Cursor.Current = Cursors.WaitCursor;
                        string ActionType_exist = "four";
                        ...
                        if (already_exist.Read())
                        {
                            ...
                            string exist_maintbl_p4 = already_exist["found_maintable"].ToString();
                            string exist_temtbl_p4 = already_exist["found_temp"].ToString();
                            ...
                            if (exist_temtbl_p4 == "0" && exist_temp_lotinfo_temp == "0")
                            {
                                already_exist.Close();
                                helper.CloseConnection();
                                // Printed date get and use insert
                                string printed_date = string.Empty;
                                ...
                                if (exist_maintbl_p4 != "0")
                                {
                                    update_enable_status = "Yes";
                                }
                                else if (exist_maintbl_p4 == "0")
                                {
                                ...
                                {                                    
                                    ...
                                    ((FormProductionInput)Owner).dynamic_data_add_gridview(this.Text, txtPartNo.Text, SelectedLotNumber, nowdate.ToString(), SelectedQuantity, txt_patern4_qty.Text, SelectedManfDate, btnId, string.Empty, txtLotNo.Text, string.Empty,Material_code_selected, Bproduct_p4, Onhold_p4, Scrap_p4, reason_hs_p4);
                                    this.Close();
                                }
                                else
                                {
